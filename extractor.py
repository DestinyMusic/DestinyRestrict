...
PASSWORD_ERROR_MESSAGE = "Password needed for {}"

def is_share_link(url): return False
def is_url_shortener(url): return False
def bypass_shortener(url): return url

def speed_string_to_bytes(size_text):
    size_text = str(size_text).lower().strip()
    if not size_text: return 0
    units = {"b": 1, "k": 1024, "m": 1048576, "g": 1073741824, "t": 1099511627776}
    for u, m in units.items():
        if size_text.endswith(u) or size_text.endswith(u+"b"):
            import re
            return int(float(re.sub(r'[A-Za-z]+', '', size_text)) * m)
    try: return int(float(size_text))
    except: return 0
